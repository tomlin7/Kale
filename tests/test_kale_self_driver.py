...
def test_self_hosting_compiler_driver():
    ...
    try:
        ...
        for tmp_file in [
            ...
            "bin/driver_err_syn.kl",
            "bin/driver_err_sem.kl",
            "bin/out.c",
        ]:
...
def test_self_hosting_compiler_binary_execution():
    """src/kale_self/main.kl: Standalone kalec.exe compiles and displays usage banner"""
    os.makedirs("bin", exist_ok=True)
    exe_path = "bin/kalec.exe"
    try:
        r = subprocess.run(
            ["uv", "run", "--directory", "E:/kale", "python", "-m", "kale.cli", "build",
             "src/kale_self/main.kl", "-o", exe_path],
            capture_output=True, text=True, cwd="E:/kale"
        )
        assert r.returncode == 0, f"Compile kalec.exe failed:\n{r.stdout}\n{r.stderr}"
        assert os.path.isfile(exe_path)

        # Run kalec.exe with no args (default entry point)
        r2 = subprocess.run([exe_path], capture_output=True, text=True, cwd="E:/kale")
